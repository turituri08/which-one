...
- [x] `ShufflePlan`について「保持手は常に1本」「`transfer`以外で保持手が変わらない」をユニットテストで検証する。
- [x] Phase 1のフル検証（`dart format .`、`flutter analyze`、`flutter test`）を実行する。
...
| 8      | `LevelShufflePlanner`が生成する`ShufflePlan`について、Level1〜5・シード0〜19の組み合わせで`initialHolder`から`steps`を順に適用し、`transfer`だけが保持手を変更し、結果が`finalHolder`と一致することを検証するユニットテストを追加。Phase 1完了ゲートとしてフル検証（`dart format .`、`flutter analyze`、`flutter test`）を実行し、Phase 1のステータスをCompletedに更新。初回の`dart format .`実行で、エディタの個人設定（100文字幅）とCLIデフォルト（80文字幅）の不一致による大量差分が判明したため、`analysis_options.yaml`に`formatter: page_width: 100`を追加し、両者を一致させた | `dart format .`（`page_width: 100`設定後）: 2 files changed（`app_theme.dart`/`home_screen.dart`、フォーマットのみ）/ `flutter analyze`: No issues found / `flutter test`: 34 passed |
