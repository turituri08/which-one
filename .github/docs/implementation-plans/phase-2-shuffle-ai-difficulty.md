...
- [x] プレイテストを実施し、正答率・見失う理由・再挑戦意欲を記録する。（ユーザーが実機/シミュレータで実施。定性的なフィードバックを記録。プレイテストはこの1回で終わりにせず、以後も継続的に実施する方針）
- [x] Phase 2のフル検証（`dart format .`、`flutter analyze`、`flutter test`）を実行する。
- [x] 計画書のチェックボックスと実施記録を更新できる状態にする。
- 対象ファイル：
...
| 8.1 | ユーザーが実施したプレイテストのフィードバックを反映した暫定値調整。(1) `LeftRightStepSequenceBuilder.baseRepetitions`を2→4に引き上げ、Level 1から手数が少なすぎるとの指摘に対応。(2) `CrossPauseStepSequenceBuilder`/`CrossFeintStepSequenceBuilder`の`tempoVariationRatio`を0.4→0.25に引き下げ、緩急の「遅い」側が遅すぎるとの指摘に対応。(3) `HandsPainter`の`move`移動範囲（縦方向）を`_moveMinY`/`_moveMaxY`＝0.15〜0.85から0.08〜0.92へ拡大し、縦画面をより活かすよう変更。(4) `transfer`の接触点での静止時間を、Level 1で300ms・Level 9以上で120msへ線形に短くする`_transferHoldDurationFor(level)`を追加（従来は全レベル一律250ms固定）。「静止した回数を数えるだけで正解できてしまう」という指摘への緩和策で、根本解決はPhase 5の複数演者化を待つ前提。`HandsPainter`に`level`パラメータを追加し、`HandsView`から`state.level`を渡すよう変更。いずれも暫定値であり、今後もプレイテストで調整する。既存のクラッシュ確認・elapsed確認・戻りアニメーション確認は全て再実行し緑を確認（新しい数値そのものを検証する専用テストは追加していない）。 | `flutter test` 全83件成功、`flutter analyze` 指摘なし |
| 8.2 | ユーザーの指摘により、8.1で`LeftRightStepSequenceBuilder.baseRepetitions`のみ2→4に変更し、`CrossPauseStepSequenceBuilder`/`CrossFeintStepSequenceBuilder`の`baseRepetitions`を据え置いていたことによる回帰を発見・修正。band間で`baseRepetitions`が揃っていなかったため、Level 3（6往復）→Level 4（5往復）で往復回数が減る逆転が発生していた。3つのbuilder全ての`baseRepetitions`を4に統一し、Level 1〜9で4回→12回まで1ずつ滑らかに増える進行に修正（各builderのdocコメントも「Level 1での」という誤った説明から、計算式の基準値であることを説明する内容に修正）。再発防止のため、Level 1〜9の往復回数（`steps.length`）がband境界を含めて単調に増えることを確認するテストを`shuffle_generator_test.dart`に追加。 | `flutter test` 全84件成功、`flutter analyze` 指摘なし |
| 8.3 | ユーザーの指示により、`CrossPauseStepSequenceBuilder.baseRepetitions`を5、`CrossFeintStepSequenceBuilder.baseRepetitions`を6へ変更（8.2で統一した4から、band別の値へ再度分離）。Level 1〜9の往復回数は4,5,6→8,9,10→12,13,14となり、新しい難易度帯（交差・フェイントの解禁）に入るタイミングで一段多く増える進行になる。band境界（Level3→4、Level6→7）でも減る逆転は起きないため、8.2で追加した単調増加の回帰テストはそのまま緑。 | `flutter test` 全84件成功、`flutter analyze` 指摘なし |
| 8.4 | プレイテスト記録（ユーザーが実機/シミュレータで実施、定性的な記録）。指摘事項：(1) Level 1の手数が少なすぎる（→8.1〜8.3でbaseRepetitionsを調整）、(2) 緩急の「遅い」側が遅すぎる（→8.1でtempoVariationRatioを調整）、(3) 縦画面なのでmoveはもっと縦幅を使ってよい（→8.1でmoveの縦範囲を拡大）、(4) transferの静止時間が一定だと、静止回数を数えるだけで正解できてしまう＝暗記ではなく単純化されたパターンで見破れてしまう（→8.1でレベルに応じて静止時間を短縮。手が2本のうちは根本解決にならず、Phase 5の複数演者化で緩和される見込みという認識で一致）。「正答率」の具体的な数値、「再挑戦意欲」の明示的な言及はないが、総合的には好意的な評価（「ゲーム自体は面白くなってきた」）を得ている。ユーザーの意向により、プレイテストは今回で区切りとせず今後も継続的に実施する方針とし、Commit 8はこの時点の記録で完了とする。Roadmap記載のPhase 2完了条件との対応：①暗記だけでは突破できない（`ShuffleValidator`＋シード毎に異なる生成で担保）②シードによる再現性・不正な保持手変更なし（`ShuffleGenerator`/`ShuffleValidator`のテストで担保）③「速すぎて見えない」という指摘は出ていない（③の裏返しである「フェイントに引っかかった」という明示的な言及はまだない）④Level 1〜9の難易度上昇の納得感は、8.1〜8.3の調整で向上中、との認識。 | プレイテストのため検証コマンドなし（フル検証は8で実施済み） |
