...
stateDiagram-v2
    ...
    confirming --> paused : appInactive
    shuffling --> paused : appInactive
    answering --> paused : appInactive
    paused --> confirming : appResumed (previous phase)
    paused --> shuffling : appResumed (previous phase)
    paused --> answering : appResumed (previous phase)
...
| `paused`     | 非アクティブ化前の進行状態と残り時間を保持する | 入力なし               |
...
| 音声読込失敗           | 無音で継続する。                                                                                               |
| アプリが非アクティブ化 | 現在のチャレンジ、シャッフルタイムライン、回答タイマーを一時停止する。復帰後は同じ状態・残り時間から再開する。 |
...
- アプリの非アクティブ化中に回答タイマーとシャッフルタイムラインが進まないこと
...
- バックグラウンド移行後に、同じチャレンジ状態と残り時間から再開できる
