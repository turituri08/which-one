...
| 用語           | 定義                                                                              |
| -------------- | ---------------------------------------------------------------------------------- |
| チャレンジ     | Level 1から、リトライも使い切った上での不正解まで連続して遊ぶ、1回のプレイ単位。   |
...
| リトライ       | 不正解・時間切れ時に、広告視聴と引き換えに同じレベルからやり直せる救済措置。1チャレンジにつき1回だけ使える。 |
...
- 不正解・時間切れになった場合、正解位置を提示したうえでチャレンジは終了する。ただし1チャレンジにつき1回だけ、広告視聴と引き換えに同じレベルからやり直せる（4.6参照）。この猶予を使い切った後の不正解・時間切れでチャレンジは確定的に終了する。
...
- アプリが非アクティブになってもシャッフル・回答タイマーは停止しない。理由（意図的な操作か強制的な割り込みか）を問わず、離れていた分だけ不利になる（TDD、ADR 0004）。強制的な割り込みによる失敗は、上記のリトライで救済する。
...
不正解・時間切れ時は、選んだ手（または未選択）と正解位置を明確に区別して見せる。シャッフルのリプレイは実施しない。

#### リトライ確認

このチャレンジでまだリトライを使っていない場合、正解位置の提示後に「広告を見て同じレベルからやり直しますか？」を確認する。

- 広告を視聴した場合：**同じレベル**のコイン位置確認へ戻る。シャッフルは失敗時とは異なる乱数で再生成し、直前の失敗と相関する形で正解を推測できないようにする（例：2択で前回の逆を選べば必ず正解する、といった状態を作らない。7.4参照）。レベル・最高到達レベルは変更しない。
- 広告を視聴しない、または広告が読み込み失敗・表示不可の場合：リトライは提示せず、そのまま結果画面へ進む（広告失敗時にゲーム進行を止めない既存方針に従う）。
- このチャレンジで既にリトライを使用済みの場合：正解位置の提示後、リトライは提示せずそのまま結果画面へ進む。

リトライの可否はチャレンジ全体で1回のみとし、レベルごとにはリセットされない。
...
結果画面にランキング、共有、称号、詳細な統計は表示しない。`RETRY`またはタイトルへ戻る操作を選んだ後に広告を表示し、広告終了後に選択した遷移先へ進む。この広告は、リトライ確認時の広告とは別枠（毎回表示）である。ランキングはタイトルのアイコンから確認する。
...
| 4–6      | 演者1人・手2本       | 上下・曲線・短い停止・交差を加える。                                        |
| 7–9      | 演者1人・手2本       | 横切り、緩急、見せかけの動きを加える。                                      |
...
- リトライ時のシャッフルは、直前の失敗時のシャッフルと異なる乱数で生成し、失敗した回答から正解を推測できないようにする。
...
- 収益モデルは広告のみ。課金要素（アイテム課金、チケット購入等）は設けない。
- 広告は次の2箇所でのみ表示する。
  1. 不正解・時間切れ時のリトライ確認（4.6参照）。1チャレンジにつき最大1回。
  2. チャレンジの結果画面で再挑戦またはタイトルへ戻る操作を選んだ後。毎回1回。
- シャッフル中、コイン位置の確認中、回答中、正解後のレベル移行中には広告を表示しない。
- 広告視聴を条件にしたリトライは、1チャレンジにつき1回までとする。ヒントや直接的な報酬（コイン位置の示唆等）を広告と引き換えに提供することはしない。
- 広告が読み込み失敗・表示不可の場合は、いずれの箇所でもゲーム進行を止めない。リトライ確認では提示せずそのまま結果画面へ、結果画面後の広告では広告なしで選んだ遷移先へ進む。
- 広告表示後は、広告表示前にユーザーが選んだ遷移先（同じレベルでのリトライ、新しいチャレンジ、またはタイトル）へ進む。
...
- 手の直接タップによる回答
- 制限時間付きの回答フェーズ
- 不正解・時間切れ時の広告視聴リトライ（1チャレンジにつき1回）
...
- リトライ確認画面のUI・演出（専用画面か、結果画面遷移前のモーダルか等）。
- リトライ用の広告が一時的に読み込めない場合、再提示の猶予を設けるか、その場で結果画面へ進むだけにするか。
