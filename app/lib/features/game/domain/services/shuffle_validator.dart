...
///
/// 検証するのは、ゲームが成立するために欠かせない次の3点だけに絞る。
/// - 最低1回の`transfer`があること（コインが一度も動かないと観察する意味がない）
...
class ShuffleValidator {
  ...
  bool isValid(ShufflePlan plan) {
    final bool hasTransfer = plan.steps.any(
      (step) => step.type == ShuffleStepType.transfer,
    );
    if (!hasTransfer) {
      return false;
    }
